        ...
        try {
            ...
            for (const file of files) {
                if (!file.isDir && file.modifiedAt) {
                    ...
                    if (`${d.getUTCFullYear()}-${d.getUTCMonth()}` !== `${dModified.getUTCFullYear()}-${dModified.getUTCMonth()}`) {
                        ...
                        this.adapter.log.debug(`[removeOldCacheFiles] Keeping cached file: ${file.file} from ${dModified.toISOString()}`);
                    }
                }
            }
        } catch (err) {
