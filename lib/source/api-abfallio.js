...
class SourceApiAbfallIo extends BaseSource {
    ...
    async getApiStreets(cityId) {
        if (cityId) {
            return await this.getApiCached(`https://api.abfall.io/?key=${cityId}&modus=${MODE_KEY}&waction=init`, `abfallio-streets-${cityId}`);
        } else {
    ...
    async getApiTypes(cityId) {
        if (cityId) {
            return await this.getApiCached(`https://mymuell.jumomind.com/mmapp/api.php?r=trash&city_id=${cityId}`, `abfallio-types-${cityId}`);
        } else {
    ...
    async getApiPickups(cityId, areaId) {
        if (cityId) {
            const pickupData = await this.getApiCached(`https://mymuell.jumomind.com/webservice.php?idx=termins&city_id=${cityId}&area_id=${areaId}&ws=3`, `abfallio-pickups-${cityId}-${areaId}`, true);
