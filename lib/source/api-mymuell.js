...
class SourceApiMymuell extends BaseSource {
    ...
    async validate() {
        ...
        const streetId = this.adapter.config.apiMymuellStreetId;

        if (cityId && streetId && streetId.includes('-')) {
            return true;
        } else {
            this.adapter.log.info(`[api-mymuell] no city or street configured`);
            return false;
        }
    }
    ...
    async getPickupDates() {
        ...
        this.adapter.log.debug(`(0) [api-mymuell] update started by api - cityId: "${cityId}" - streetId: "${streetId}" - areaId: "${areaId}"`);
