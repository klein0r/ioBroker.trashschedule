...
class SourceIcal extends BaseSource {
    ...
    async validate() {
        ...
        if (iCalInstance) {
                ...
                if (instanceObject && typeof instanceObject === 'object') {
                    ...
                    if (typeof instanceObject.native === 'object') {
                        ...
                        this.adapter.log.info(
                            `[ical] configured ical preview is ${daysPreview} days (until ${maximumPreviewDateFormat}) - increase this value to find more events in the future`,
                        );
            ...
            this.adapter.log.info(`[ical] no ical instance configured`);
        }
        ...
        try {
            ...
            if (iCalDataState && iCalDataState.val) {
                this.adapter.log.debug(
                    `(0) [ical] update started by foreign state value "${iCalInstance}.data.table" - lc: ${new Date(iCalDataState.lc).toISOString()} - ts: ${new Date(iCalDataState.ts).toISOString()}`,
                );
                ...
                if (typeof stateVal === 'string') {
                    try {
                        ...
                        this.adapter.log.error(`(0) [ical] unable to parse iCal json: ${err.toString()}`);
                ...
                if (stateVal && Array.isArray(stateVal) && stateVal.length > 0) {
                    this.adapter.log.debug(
                        `(0) [ical] found ${stateVal.length} events in state "${iCalInstance}.data.table"`,
                    );
                    ...
                    this.adapter.log.warn(`(0) [ical] found no events in state "${iCalInstance}.data.table"`);
                }
