            ...
            try {
                ...
                if (instanceObject && typeof instanceObject === 'object') {
                    ...
                    if (typeof instanceObject.native === 'object') {
                        ...
                        this.adapter.log.info(`[ical] configured ical preview is ${daysPreview} days (until ${maximumPreviewDateFormat}) - increase this value to find more events in the future`);
    ...
    async getPickupDates() {
        try {
            ...
            if (iCalDataState && iCalDataState.val) {
                this.adapter.log.debug(`(0) [ical] update started by foreign state value - lc: ${new Date(iCalDataState.lc).toISOString()} - ts: ${new Date(iCalDataState.ts).toISOString()}`);
