...
class SourceApiAwido extends BaseSource {
     ...
     *
     * @returns {Promise<boolean>} whether the adapter configuration is valid
     */
    async validate() {
     ...
     * @returns {Promise<*[]>} pickup data
     */
    async getApiPickups(provider, cityId, streetId, typeIds) {
        if (provider && cityId && streetId && typeIds) {
            const interestData = `[{"placeId":"${cityId}","districtId":"${streetId}","fractionIds":${
                typeIds
            },"categoryIds":[]}]`;
            ...
            if (additionalEventData && Array.isArray(additionalEventData) && additionalEventData.length !== 0) {
                additionalEventData.forEach(pickup => {
                    pickupData.push({
                        date: pickup.date,
                        name: pickup.title,
                        description: pickup.subtitle,
