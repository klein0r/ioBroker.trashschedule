...
class SourceApiJumomind extends BaseSource {
    ...
    async getPickupDates() {
        ...
        this.adapter.log.debug(
            `(0) [api-jumomind] update started by api - cityId: "${cityId}" - streetId: "${streetId ?? 'n/a'}" - areaId: "${areaId}"`,
        );
    ...
    async getApiCities(provider) {
        const citiesData = await this.getApiCached(
            `https://${provider}.jumomind.com/mmapp/api.php?r=cities`,
            `jumomind-cities-${provider}.json`,
        );
    ...
    async getApiStreets(provider, cityId) {
        if (cityId) {
            const streetsData = await this.getApiCached(
                `https://${provider}.jumomind.com/mmapp/api.php?r=streets&city_id=${cityId}`,
                `jumomind-streets-${provider}-${cityId}.json`,
            );
    ...
    async getApiTypes(provider, cityId) {
        if (cityId) {
            const typesData = await this.getApiCached(
                `https://${provider}.jumomind.com/mmapp/api.php?r=trash&city_id=${cityId}`,
                `jumomind-types-${provider}-${cityId}.json`,
            );
