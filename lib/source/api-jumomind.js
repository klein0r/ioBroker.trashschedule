...
class SourceApiJumomind extends BaseSource {
    ...
    async getApiCities(provider) {
        const streetsData = await this.getApiCached(`https://${provider}.jumomind.com/mmapp/api.php?r=cities`, `jumomind-cities-${provider}.json`);
        return JSON.parse(streetsData);
    }

    async getApiStreets(provider, cityId) {
        if (cityId) {
            const citiesData = await this.getApiCached(`https://${provider}.jumomind.com/mmapp/api.php?r=streets&city_id=${cityId}`, `jumomind-streets-${provider}-${cityId}.json`);
