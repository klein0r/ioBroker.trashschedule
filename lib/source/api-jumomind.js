...
class SourceApiJumomind extends BaseSource {
    ...
    async getApiProviders() {
        return Object.keys(providers).map(id => ({ id, name: providers[id].url }));
    }

    async getApiCities(provider) {
        return await this.getApiCached(`https://${provider}.jumomind.com/mmapp/api.php?r=cities`, `jumomind-cities-${provider}`);
    }

    async getApiStreets(provider, cityId) {
        if (cityId) {
            return await this.getApiCached(`https://${provider}.jumomind.com/mmapp/api.php?r=streets&city_id=${cityId}`, `jumomind-streets-${provider}-${cityId}`);
        } else {
    ...
    async getApiTypes(provider, cityId) {
        if (cityId) {
            return await this.getApiCached(`https://${provider}.jumomind.com/mmapp/api.php?r=trash&city_id=${cityId}`, `jumomind-types-${provider}-${cityId}`);
        } else {
    ...
    async getApiPickups(provider, cityId, areaId) {
        if (cityId) {
            const pickupData = await this.getApiCached(
                ...
                `jumomind-pickups-${provider}-${cityId}-${areaId}`,
                true,
            );
